...
def convert_png_to_tiff(input_file):
    """Convert a PNG file to 64x64 TIFF format matching Cinema 4D output style."""
    ...
    cmd = [
        ...
        '-colorspace', 'sRGB',
        '-alpha', 'on',  # Ensure alpha channel is preserved
        '-compress', 'RLE',
        '-depth', '8',
        '-units', 'PixelsPerInch',
        '-density', '72x72',  # Set 72 DPI resolution
        '-define', 'tiff:rows-per-strip=32',  # Match original strip size
        '-set', 'tiff:software', 'Cinema 4D',  # Add Cinema 4D metadata
        output_file
    ]
